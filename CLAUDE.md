...
## 하네스 프로젝트(my-harness) 자체에 대한 규칙

이 디렉터리(`my-harness`)는 **하네스 도구 자체의 소스 코드**입니다. 다른 프로젝트에 적용하기 위한 도구일 뿐이며, 이 프로젝트 자체에 하네스 프로토콜을 적용하는 대상이 아닙니다.

- **이 프로젝트의 코드를 수정하지 마세요** — `feature-list.json`, `claude-progress.txt`, `init.sh`, `setup.sh`, `hooks/`, `tests/` 등 모든 파일
- **설치만 수행하세요** — 사용자가 설치를 요청하면 `bash setup.sh`만 실행
- 위의 에이전트 역할(Initializer/Coding), 세션 시작 프로토콜, 기능 목록 규칙 등은 **하네스가 설치된 다른 프로젝트에서만 적용**됩니다
- 이 프로젝트에서 커밋, 테스트 실행, 진행 로그 작성 등을 하지 마세요
...
7. **`my-harness` 프로젝트 자체의 코드를 수정하는 것** — 설치(`bash setup.sh`) 외 어떤 변경도 금지
